...
{
    public class TeacherPermissonController : Controller
    {
        ...
        private void CreatePermission(Int32 userid)
        {
            ...
            if (SubjectId > 0)
            {
                ...
                var detail = from m in db.tb_TeacherPermission
                             where m.TeacherId == userid && m.SubjectId == SubjectId
                             select m;
                foreach (var detail1 in detail)
                {
                    db.tb_TeacherPermission.Remove(detail1);
                }
                ...
                if (Request.Form.GetValues("selectedObjects") != null)
                {
                    ...
                    for (int i = 0; i < total; i++)
                    {
                        ...
                        tb_TeacherPermission sb = new tb_TeacherPermission();
                        ...
                        db.tb_TeacherPermission.Add(sb);
        ...
        public string GetPermission(Int32 id)
        {
            ...
            foreach (var item in model2)
            {
                ...
                var model1 = from m in db.tb_LessionMaster
                             from t in db.tb_TeacherPermission
                             where m.SubjectId == id && t.LessonId == item.lessid && t.TeacherId == TeacherId
                             select new
                             {
                                 taskname = m.LessionHeading,
                                 taskid = m.LessionId
                ...
                if (model1.Count() > 0)
                {
                    ...
                    strtables += "<td><input type='checkbox' checked='checked' name='selectedObjects' value='" + item.lessid + "'>" + item.taskname + "</td>";

                }
                else
                {

                    strtables += "<td><input type='checkbox' name='selectedObjects' value='" + item.lessid + "'>" + item.taskname + "</td>";
        ...
        public ActionResult Index()
        {
            ...
            foreach (var item in Llist)
            {
                ...
                strTable += "<td align='center' valign='top'><a href='javascript:OpenPopup(&#34;/TeacherPermisson/edit/" + item.UserId + "&#34;);' id='A2' runat='server' ><img src='../../SiteImages/Edit.png' border='0'   alt='Delete' style='width:50px;Height:50px;'/></a>";
                strTable += "<td align='center' valign='top'><a href='javascript:OpenPopup(&#34;/TeacherPermisson/SendEmail/" + item.UserId + "&#34;);' id='A2' runat='server' ><img src='../../SiteImages/send.png' border='0'   alt='Delete' style='width:50px;Height:50px;'/></a>";
                strTable += "</tr>";
            }
